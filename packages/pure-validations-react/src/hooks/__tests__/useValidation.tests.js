...
import { isValid, Validator, Validation, validate, logTo, filterFields, __clearMocks } from '@totalsoft/pure-validations'
...
describe("useValidation hook", () => {
    ...
    it("returns failure when validation result is failure", () => {
        // Arrange
        const validator = Validator.of(Validation.Failure(["baa"]));
        const model = {}
        let returnedIsValid = undefined;
    ...
    it("returns success when validation is not run", () => {
        // Arrange
        const validator = Validator.of(Validation.Failure());
    ...
    it("changes the validation function when the validator changes", () => {
        // Arrange
        const callback = () => useValidation(Validator.of(Validation.Success()), { logger: {} })
